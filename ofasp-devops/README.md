...
### 🎨 NEW: Interactive Pipeline Flow Visualizer
- **실시간 파이프라인 시각화**: CI/CD 프로세스의 모든 단계를 실시간으로 시각화
- **인터랙티브 노드**: 각 단계별 상세 정보 표시 및 클릭 가능한 인터페이스
- **애니메이션 효과**: 파이프라인 진행 상황을 실시간으로 표현
- **시뮬레이션 모드**: 파이프라인 실행 과정을 시뮬레이션으로 체험
- **상태별 색상 표시**: 성공(녹색), 실행중(파란색), 실패(빨간색), 대기중(회색)
- **실시간 데이터 연동**: 5초마다 자동으로 파이프라인 상태 업데이트
- **피드백 루프 시각화**: 실패 시 소스 컨트롤로 돌아가는 피드백 경로 표시
...
- `GET /api/pipeline-flow-status` - 파이프라인 플로우 실시간 상태 (NEW)
