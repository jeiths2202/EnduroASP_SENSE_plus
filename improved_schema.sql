...
-- 3. 오브젝트 테이블 (개선: volume_id, library_id, object_name 조합으로 UNIQUE)
CREATE TABLE aspuser.objects (
    ...
    -- 추가 제약조건: library_id는 해당 volume_id와 일치해야 함
    CONSTRAINT fk_objects_volume_library
        FOREIGN KEY (volume_id, library_id) REFERENCES aspuser.libraries(volume_id, library_id) ON DELETE CASCADE
);

-- 4. 프로그램 테이블 (기존과 동일, 참조만 수정)
...
-- 스키마 개선점 요약:
-- 1. libraries 테이블: UNIQUE(volume_id, library_name) - 볼륨별로 라이브러리 중복 허용
-- 2. objects 테이블: volume_id 컬럼 추가 및 UNIQUE(volume_id, library_id, object_name) - 전체 계층 경로에서 고유성 보장
-- 3. 외래키 제약조건으로 데이터 무결성 보장
-- 4. 성능 최적화를 위한 인덱스 추가
-- 5. 계층 구조를 명확히 반영한 제약조건 설계